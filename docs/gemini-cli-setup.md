# Using SkillEngine with Gemini CLI
...
gemini skills install /path/to/SkillEngine/skills/
```

**Install for a specific workspace only:**

```bash
gemini skills install /path/to/SkillEngine/skills/ --scope workspace
```
...
cat /path/to/SkillEngine/skills/incremental-implementation/SKILL.md > GEMINI.md
echo -e "\n---\n" >> GEMINI.md
cat /path/to/SkillEngine/skills/code-review-and-quality/SKILL.md >> GEMINI.md
```
...
To replicate the `SkillEngine` experience from other tools, you can configure a `SessionStart` hook that reminds you of the available skills or loads a meta-skill.
